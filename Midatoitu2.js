...
let salvestatud = JSON.parse(sessionString);

let salvestatudid = [];
salvestatud.forEach((salvestatu) => {
  salvestatudid.push(salvestatu.id);
});

let tabeliloedur1 = 0;
let tabeliloedur2 = 0;

if (salvestatud.length > 0) {
  for (i = 0; i < Retseptid.length; i++) {
    let poleAsju = [];
    let sarnasus = 0;

    for (j = 0; j < Retseptid[i].koostisosad.length; j++) {
      let kontroll = 0;
      salvestatudid.forEach((salvestatu) => {
        if (Retseptid[i].koostisosad[j] === salvestatu) {
          kontroll = 1;
          sarnasus++;
        }
      });
      if (kontroll !== 1) {
        poleAsju.push(Retseptid[i].koostisosad[j]);
      }
    }
    if (poleAsju.length <= 1) {
      x = "retseptitabel1";
      tabeliloedur1++;
      teeRetseptiKast(i, x, poleAsju, tabeliloedur1);
    } else if (poleAsju.length <= 3) {
      x = "retseptitabel2";
      tabeliloedur2++;
      teeRetseptiKast(i, x, poleAsju, tabeliloedur2);
    }
  }
} else {
  document.getElementsByClassName("tekst")[0].style.display = "none";
  document.getElementsByClassName("tekst")[1].style.display = "none";
  for (i = 0; i < Retseptid.length; i++) {
    x = "retseptitabel1";
    tabeliloedur1++;
    poleAsju = 0;
...
function teeRetseptiKast(i, rt, midaPole, loendur) {
  let vretseptikast = document.createElement("div");
  vretseptikast.setAttribute("class", "vretseptikast");
  vretseptikast.setAttribute("id", loendur);
  document.getElementsByClassName(rt)[0].appendChild(vretseptikast);

  let sretseptikast = document.createElement("div");
  sretseptikast.setAttribute("class", "sretseptikast");
  document
    .getElementsByClassName(rt)[0]
    .getElementsByClassName("vretseptikast")
    [loendur - 1].appendChild(sretseptikast);

  let img = document.createElement("img");
  img.src = Retseptid[i].pilt;
  document
    .getElementsByClassName(rt)[0]
    .getElementsByClassName("vretseptikast")
    [loendur - 1].getElementsByClassName("sretseptikast")[0]
    .appendChild(img);

  let nimi = document.createElement("div");
  nimi.setAttribute("class", "toiduNimetus");
  nimi.textContent = Retseptid[i].retseptiNimi;
  document
    .getElementsByClassName(rt)[0]
    .getElementsByClassName("vretseptikast")
    [loendur - 1].getElementsByClassName("sretseptikast")[0]
  ...
  let asjad = document.createElement("div");
  if (rt === "retseptitabel1") {
    asjad.textContent = "Koostisosad: ";
    for (j = 0; j < Retseptid[i].koostisosad.length; j++) {
      toiduained.forEach((toiduaine) => {
        if (Retseptid[i].koostisosad[j] === toiduaine.id) {
          asjad.textContent += toiduaine.Nimetus;
          if (j < Retseptid[i].koostisosad.length - 1) {
            asjad.textContent += ", ";
          }
        }
      });
    }
  } else if (rt === "retseptitabel2") {
    asjad.textContent = "Sul on puudu: ";
    for (j = 0; j < midaPole.length; j++) {
      toiduained.forEach((toiduaine) => {
        if (midaPole[j] === toiduaine.id) {
          asjad.textContent += toiduaine.Nimetus;
          if (j < midaPole.length - 1) {
            asjad.textContent += ", ";
          }
        }
      });
  ...
  asjad.setAttribute("class", "retseptiAsjad");
  document
    .getElementsByClassName(rt)[0]
    .getElementsByClassName("vretseptikast")
    [loendur - 1].getElementsByClassName("sretseptikast")[0]
  ...
  let vaataRetsepti = document.createElement("div");
  vaataRetsepti.appendChild(document.createTextNode("Vaata Retsepti"));
  vaataRetsepti.setAttribute("class", "vaataRetsepti");
  vaataRetsepti.setAttribute("id", i);
  vaataRetsepti.addEventListener("click", function () {
    let andmed = Retseptid[i];
    let andmed2 = salvestatud;
    sessionStorage.setItem("andmejarjend", JSON.stringify(andmed));
    sessionStorage.setItem("andmejarjend1", JSON.stringify(andmed2));
    window.location.href = "Projekt-LK3.html";
  });
  document
    .getElementsByClassName(rt)[0]
    .getElementsByClassName("vretseptikast")
    [loendur - 1].getElementsByClassName("sretseptikast")[0]
    .appendChild(vaataRetsepti);
}

if (tabeliloedur1 === 0) {
  let x = document.getElementsByClassName("tekst")[0];
  x.style.display = "none";
  document.getElementsByClassName("retseptitabel1")[0].style.display = "none";
} else if (tabeliloedur2 === 0) {
  let x = document.getElementsByClassName("tekst")[1];
  x.style.display = "none";
  document.getElementsByClassName("retseptitabel2")[0].style.display = "none";
}
if (tabeliloedur1 === 0 && tabeliloedur2 === 0) {
  document.getElementsByClassName("tekst")[0].style.display = "none";
  document.getElementsByClassName("tekst")[1].style.display = "none";
  document.getElementsByClassName("retseptitabel1")[0].style.display = "none";
  let tekstihoiatus = document.createElement("div");
  tekstihoiatus.setAttribute("class", "tekst");
  tekstihoiatus.textContent =
    "Kahjuks ei leidnud sulle ühtegi sobivat retsepti!";
  document.getElementsByClassName("sisu")[0].appendChild(tekstihoiatus);
}
